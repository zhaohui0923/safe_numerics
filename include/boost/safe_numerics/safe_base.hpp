...
template <class Stored, Stored Min, Stored Max,
...
// type "Stored" be a scalar type.  Problem is when violates this rule, the
// error message (on clang) is "A non-type template parameter cannot have type
// ... " where ... is some non-scalar type The example which triggered this
// investigation is safe<safe<int>> .  It was difficult to make the trip from
// the error message to the source of the problem, hence we're including this
// comment here.
template <class Stored,  // 底层的存储类型
          Stored Min,    // 底层存储类型的最小值
          Stored Max,    // 底层存储类型的最大值
          class P,       // promotion polic
          class E        // exception policy
          >
class safe_base {
 private:
  ...
  friend std::basic_ostream<CharT, Traits> &operator<<(
      std::basic_ostream<CharT, Traits> &os, const safe_base &t) {
    t.output(os);
    return os;
  }
  ...
  friend inline std::basic_istream<CharT, Traits> &operator>>(
      std::basic_istream<CharT, Traits> &is, safe_base &t) {
    t.input(is);
    return is;
  }
 ...
 public:
  ...
  template <class T, typename std::enable_if<
                         std::is_convertible<T, Stored>::value, bool>::type = 0>
  ...
  template <class R,
            typename std::enable_if<!boost::safe_numerics::is_safe<R>::value,
                                    int>::type = 0>
