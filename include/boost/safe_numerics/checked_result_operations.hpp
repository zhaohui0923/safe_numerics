...
constexpr inline void display(
    const boost::safe_numerics::checked_result<T> &c) {
  switch (c.m_e) {
    ...
    case safe_numerics_error::
      ...
      std::terminate();
    case safe_numerics_error::domain_error:  // one operand is out of valid
                                             // range
      std::terminate();
    case safe_numerics_error::range_error:  // result cannot be produced for
                                            // this operation
      std::terminate();
    case safe_numerics_error::precision_overflow_error:  // result lost
                                                         // precision
      std::terminate();
    case safe_numerics_error::underflow_error:  // result is too small to be
                                                // represented
      std::terminate();
    case safe_numerics_error::negative_value_shift:  // negative value in shift
                                                     // operator
      std::terminate();
    case safe_numerics_error::negative_shift:  // shift a negative value
      std::terminate();
    case safe_numerics_error::shift_too_large:  // l/r shift exceeds variable
                                                // size
      std::terminate();
    case safe_numerics_error::uninitialized_value:  // creating of uninitialized
                                                    // value
      std::terminate();
  }
}
...
struct sum_value_type {
  ...
  template <class T>
  constexpr sum_value_type(const checked_result<T> &t) : m_flag(to_flag(t)) {}
...
typename std::enable_if<std::is_integral<T>::value,
                        checked_result<T> >::type constexpr inline
operator+(const checked_result<T> &t, const checked_result<T> &u) {
  ...
  if (safe_numerics_error::success == e) return checked::add<T>(t, u);
...
operator-(const checked_result<T> &t, const checked_result<T> &u) {
  ...
  constexpr const std::uint8_t order =
      static_cast<std::uint8_t>(value_type::count);
  ...
  const safe_numerics_error e = result[tx * order + ux];
  if (safe_numerics_error::success == e) return checked::subtract<T>(t, u);
...
// Expression: c1 < c2
// Return Type: boost::logic::tribool
// Semantics: compare the wrapped values of two checked_result instances. If the
//  values are such that the result of such a comparison cannot be reasonably
//  defined, The result of the comparison is
//  boost::logic::tribool::indeterminant.
template <class T>
constexpr boost::logic::tribool operator<(const checked_result<T> &t,
...
operator<<(const checked_result<T> &t, const checked_result<T> &u) {
  ...
  constexpr const std::uint8_t result[order * order] = {
      ...
      9,  // safe_numerics_error::positive_overflow_error),  //
          // greater_than_zero,
      9,  // safe_numerics_error::positive_overflow_error,   // greater than
          // max,
      1,  // safe_numerics_error::range_error,               // indeterminate,
          //},
          // t == indeterminate
...
inline std::basic_ostream<CharT, Traits> &operator<<(
  ...
  if (!e)
    ...
    os << std::error_code(r.m_e).message() << ':'
       << static_cast<char const *>(r);
  return os;
}
...
inline std::basic_istream<CharT, Traits> &operator>>(
  ...
  if (!e)
    ...
    is >> std::error_code(r.m_e).message() >> ':' >>
        static_cast<char const *>(r);
  return is;
}
